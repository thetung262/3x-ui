...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-arm.tar.gz https://github.com/kenhtaymay/3x-ui/releases/download/${last_version}/x-ui-linux-arm.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/kenhtaymay/3x-ui/releases/download/${last_version}/x-ui-linux-arm.tar.gz"
        echo -e "Begining to install x-ui $1"
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-arm.tar.gz ${url}
        if [[ $? -ne 0 ]]; then
    ...
    tar zxvf x-ui-linux-arm.tar.gz
    rm x-ui-linux-arm.tar.gz -f
    cd x-ui
